...
import { Breadcrumbs, Card, CardContent, CardHeader, CardTitle, Dialog, DialogContent, DialogHeader, DialogTitle, ErrorState, LoadingState, PageHeader } from '@ams/ui';
...
import { FINANCIAL_ROUTES, INVOICE_STATUS_OPTIONS } from '../constants/invoice.constants';
...
export function InvoiceDetailPage() {
  ...
  const form = useForm<CreatePaymentFormValues>({
    ...
    defaultValues: { invoiceId: id, paymentDate: new Date().toISOString().split('T')[0], method: undefined, amount: 0 },
  });

  const { mutate: recordPayment, isPending } = useMutation({
    mutationFn: (values: CreatePaymentFormValues) => paymentsApi.create(values),
    onSuccess:  () => {
      queryClient.invalidateQueries({ queryKey: financialKeys.invoices.detail(id) });
      queryClient.invalidateQueries({ queryKey: financialKeys.payments.lists() });
      setPayOpen(false);
      form.reset();
    },
  });
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        breadcrumbs={<Breadcrumbs items={[{ label: 'Dashboard', href: '/dashboard' }, { label: 'Invoices', href: FINANCIAL_ROUTES.INVOICES }, { label: invoice.invoiceNumber }]} />}
      />
